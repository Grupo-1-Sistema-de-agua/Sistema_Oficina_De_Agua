    ...
    <div class="card-body">
      ...
      <?php if (($clima['disponible'] ?? false) === true) : ?>
        <div class="d-flex align-items-center gap-3">
          ...
          <div>
            <div class="fs-3 fw-semibold"><?= esc_nativo(number_format((float) $clima['temperatura'], 1)) ?><?= esc_nativo($clima['unidad']) ?></div>
            <div class="text-muted"><?= esc_nativo($clima['descripcion']) ?></div>
          </div>
        </div>
      <?php else : ?>
        <div class="alert alert-warning mb-0"><?= esc_nativo($clima['mensaje'] ?? 'El clima no esta disponible en este momento.') ?></div>
      <?php endif; ?>
    </div>
  </div>
