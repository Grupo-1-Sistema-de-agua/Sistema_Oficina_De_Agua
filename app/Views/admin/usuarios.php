  ...
  <div class="card shadow-sm border-0">
    <div class="card-body p-0">
      <div class="table-responsive">
        <table class="table align-middle mb-0 table-responsive-cards">
          ...
          <tbody>
            <?php foreach ($usuarios as $usuario): ?>
              <tr>
                <td data-label="Nombre"><?= esc_nativo($usuario['nombre']) ?></td>
                <td data-label="Correo"><?= esc_nativo($usuario['email']) ?></td>
                <td data-label="Rol"><?= esc_nativo($usuario['rol_nombre']) ?></td>
                ...
                <td class="text-end celda-acciones" data-label="Acciones">
                  <div class="d-inline-flex gap-2 flex-wrap justify-content-end">
                    ...
                    <form action="<?= base_url('admin/usuarios/eliminar') ?>" method="post" class="d-inline" onsubmit="return confirm('Seguro que quieres eliminar este usuario?');">
                      <?= csrf_field_nativo() ?>
                      <input type="hidden" name="usuario_id" value="<?= esc_nativo($usuario['id']) ?>">
